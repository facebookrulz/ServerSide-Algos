                ...
                for mf, objArr in self.master_mf_data_list.items():
                        ...
                        if( len(chgArr) == len(phase_dt_list) ):
                                ...
                                for looper in range( len(phase_dt_list) - len(chgArr)):
                                        chgArr.append(0.0)
                                self.daily_chg_list[mf] = chgArr
                ### clear the mdf data list for the next phase run ..bad santa .. this should be done in an init function
                self.master_mf_data_list = dict()
...
for mfOuter, llOuter in dbInst.daily_chg_list.items():
        ...
        for mfInner, llInner in dbInst.daily_chg_list.items():
                  ...
                  if ( mfOuter != mfInner ):
                        ...
                        dist =  _statMod._calcQCDist(hist_A[0], hist_B[0], len(hist_A[0]))

                        distArr.append( dist )
                        #print 'dist btw '+mfOuter+' and '+mfInner+' '+str(Decimal(dist))
                  else:
                        distArr.append(0.0)
